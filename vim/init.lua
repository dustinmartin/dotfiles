...
require("lazy").setup({
	...
	-- https://github.com/ibhagwan/fzf-lua
	{
		...
		config = function()
			...
			fzf.setup({
				fzf_opts = {
					["--header"] = "[F1] Help • [F2] fullscreen • [F4] preview • [Ctrl-S] Split • [Ctrl-V] V-Split • [Ctrl-T] Tab",
					["--header-lines"] = "1",
				},
				winopts = {
					height = 0.85,
			...
			vim.api.nvim_create_autocmd("FileType", {
				...
				callback = function(event)
					vim.keymap.set(
						...
						{ buffer = event.buf, desc = "Navigate left" }
					)
					vim.keymap.set(
						"n",
						"<C-j>",
						"<cmd>TmuxNavigateDown<CR>",
						{ buffer = event.buf, desc = "Navigate down" }
					)
					vim.keymap.set(
						"n",
						"<C-k>",
						"<cmd>TmuxNavigateUp<CR>",
						{ buffer = event.buf, desc = "Navigate up" }
					)
					vim.keymap.set(
						"n",
						"<C-l>",
						"<cmd>TmuxNavigateRight<CR>",
						{ buffer = event.buf, desc = "Navigate right" }
					)
				end,
			})
		end,
