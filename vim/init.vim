" Author: Dustin Martin <dm@dustinmartin.net>
" Source: https://github.com/dustinmartin/dotfiles

source $HOME/dotfiles/vim/config/plugins.vim
source $HOME/dotfiles/vim/config/functions.vim
source $HOME/dotfiles/vim/config/settings.vim
source $HOME/dotfiles/vim/config/autocmds.vim
source $HOME/dotfiles/vim/config/coc.vim
source $HOME/dotfiles/vim/config/whichkey.vim
source $HOME/dotfiles/vim/config/mappings.vim
source $HOME/dotfiles/vim/config/commands.vim

let g:UltiSnipsExpandTrigger="<c-j>"
